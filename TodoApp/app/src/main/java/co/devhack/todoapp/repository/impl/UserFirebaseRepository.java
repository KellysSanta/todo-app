...
public class UserFirebaseRepository implements UserRepository {
    ...
    mAuth.createUserWithEmailAndPassword(user.getEmail(), user.getPassword()).addOnCompleteListener(new OnCompleteListener<AuthResult>() { //Crear usuario en firebase
        @Override
        public void onComplete(@NonNull Task<AuthResult> task) {
            if(task.isSuccessful() && task.getResult() != null){ //Validar si la tarea de creación del usuario se creo correctamente
                FirebaseUser firebaseUser = task.getResult().getUser();
